...
export default function tiptap_EntityTextNodeView(props: any): any {
  const { type } = props.node.attrs || 'UNKNOWN';
  const { text_content } = props.node.attrs || 'UNDEFINED';
  console.log('EntityText', props);
  console.log('inner text', text_content);
  ...
  return (
    <NodeViewWrapper as="span" className="react-component">
      <Menu shadow="md" width={200}>
        <Menu.Target>
          <Text variant="gradient" gradient={entity.gradient} p={0} m={0} span fw={500}>
            {`${text_content}`}
          </Text>
        </Menu.Target>
